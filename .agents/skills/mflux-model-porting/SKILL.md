...
6. **Pre-merge polish (after core port works)**
   ...
   - **img2img**: verify latent packing/normalization on the img2img path matches txt2img and training (especially when reusing a shared VAE from another model family). `pack_latents` must accept the 5D `(B, C, 1, H, W)` tensor that tiled VAE encode (`vae_encode_tiled`) returns — squeeze the temporal axis first with the bare `if latents.ndim == 5: latents = latents[:, :, 0, :, :]` form used by the `z_image`/`fibo` `pack_latents` (the upstream convention; prefer it over the guarded `ndim == 5 and shape[2] == 1` variant). A 4D-only unpack (or a bare passthrough `pack_latents`) breaks tiled img2img. This is reachable via `--low-ram`: `MemorySaver` sets `tiling_config = TilingConfig()` (`vae_encode_tiled=True`), so always test img2img with `--low-ram`, not just the default path (which is safe only because `VAEUtil.encode` squeezes 5D→4D when tiling is off).
